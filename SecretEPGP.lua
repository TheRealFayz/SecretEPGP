...
local L = AceLibrary("AceLocale-2.2"):new("SecretEPGP")
sepgp.VARS = {
  basegp = 100,
  minep = 0,
  baseaward_ep = 100,
...
sepgp.VARS.reservecall = string.format(L["{SecretEPGP}Type \"+\" if on main, or \"+<YourMainName>\" (without quotes) if on alt within %dsec."],sepgp.VARS.timeout)
sepgp._playerName = (UnitName("player"))
local out = "|cff9664c8SecretEPGP:|r %s"
...
local admincmd, membercmd = {type = "group", handler = sepgp, args = {
    ...
    restart = {
      type = "execute",
      name = L["Restart"],
      desc = L["Restart SecretEPGP if having startup problems."],
      func = function() 
        sepgp:OnEnable()
...
function sepgp:OnEnable() -- PLAYER_LOGIN (2)
  ...
  sepgp.extratip = (sepgp.extratip) or CreateFrame("GameTooltip","SecretEPGP_tooltip",UIParent,"GameTooltipTemplate")
  sepgp._versionString = GetAddOnMetadata("SecretEPGP","Version")
  sepgp._websiteString = GetAddOnMetadata("SecretEPGP","X-Website")
...
function sepgp:AceEvent_FullyInitialized() -- SYNTHETIC EVENT, later than PLAYER_LOGIN, PLAYER_ENTERING_WORLD (3)
  ...
  if not self:IsEventScheduled("SecretEPGPChannelInit") then
    self:ScheduleEvent("SecretEPGPChannelInit",self.delayedInit,delay,self)
  end
...
function sepgp:delayedInit()
  ...
  self._options = self:buildMenu()
  self:RegisterChatCommand({"/shooty","/sepgp","/SecretEPGP"},self.cmdtable())
...
function sepgp:AddDataToTooltip(tooltip,itemlink,itemstring,is_master)
  ...
  if (tooltip:NumLines() < line_limit) then
    tooltip:AddLine(" ")
    tooltip:AddDoubleLine("|cff9664c8SecretEPGP|r",textRight)
    ...
    sepgp.extratip:SetText("|cff9664c8SecretEPGP|r")
...
function sepgp:simpleSay(msg)
  SendChatMessage(string.format("SecretEPGP: %s",msg), sepgp_saychannel)
end
...
function sepgp:adminSay(msg)
  ...
  SendChatMessage(string.format("SecretEPGP: %s",msg),"OFFICER")
  -- end
end
...
function sepgp:my_epgp(use_main)
  GuildRoster()
  self:ScheduleEvent("SecretEPGPRosterRefresh",self.my_epgp_announce,3,self,use_main)
end
...
function sepgp:captureReserveChatter(text, sender, _, _, _, _, _, _, channel)
  ...
  local q = string.find(text,L["^{SecretEPGP}Type"])
    ...
    if (link_found) then
      ...
      if (quality >= 3) then
        if (IsRaidLeader() or self:lootMaster()) and (sender == self._playerName) then
          ...
          self:ScheduleEvent("SecretEPGPBidTimeout",self.clearBids,300,self)
          running_bid = true
...
function sepgp:clearBids(reset)
  ...
  sepgp.bids_off = {}
  bids_blacklist = {}
  if self:IsEventScheduled("SecretEPGPBidTimeout") then
    self:CancelScheduledEvent("SecretEPGPBidTimeout")
  end
  running_bid = false
