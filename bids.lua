...
function sepgp_bids:countdownFinish(reset)
  if self:IsEventScheduled("SecretEPGPBidCountdown") then
    self:CancelScheduledEvent("SecretEPGPBidCountdown")
  end
  self._counter = 6
  if (reset) then
...
function sepgp_bids:bidCountdown()
  ...
  self:ScheduleRepeatingEvent("SecretEPGPBidCountdown",self.countdownCounter,1,self)
  self:ScheduleEvent("SecretEPGPBidCountdownFinish",self.countdownFinish,6,self)
end
